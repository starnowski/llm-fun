...
- **Populate Database**:
  ...
  - If empty, iterate through the `situations` array.
  - Generate the embedding vector for each situation's name.
  - Insert the document into the RxDB collection.
- **Notify User**: Once all embeddings are stored, update the UI to display: *"Wszystkie sytuacje zostały zapisane w bazie danych"* (All situations have been stored in the database), and enable the input fields.

### Step 3: Semantic Search Implementation
- Modify the `processInput` function:
  - When the user submits text, generate the embedding vector for their input using Transformers.js.
  - Query the RxDB collection using vector search capabilities (e.g., finding the nearest neighbor using cosine similarity).
  - Define a similarity threshold (e.g., `0.85`).
  - If the nearest neighbor's similarity score is above the threshold, consider it a match, add the points, and update the UI.
  - If no match is above the threshold, treat it as a miss (display `❌`).

### Step 4: Refactoring & Cleanup
- Remove the old `normalizeString` exact-matching logic.
- Ensure all asynchronous operations (loading the model, querying the DB) are properly handled with `async/await` and that the UI remains responsive.
